...
const Profile = () => {
  ...
  //   }
  //   try {
  //     await axios.delete("http://localhost:3001/api/users/" + user._id,deleteUser
  //       );
    ...
    <>
      ...
      (<div className="flex flex-col items-center">
        <label
          ...
          >
            <img src={file? URL.createObjectURL(file) : UserImage+user.profilePic} alt="CLICK HERE!" className="profilepicUpload"></img>
          </label>
          <input
            className="hidden"
            type="file"
        ...
        <div className="flex flex-col items-center">
        <p className="text-xl font-rampart mt-20">
          Username - <span className="uppercase">{user.username}</span>
        </p>
        <p className="text-xl font-rampart mt-5">
          Email - <span className="uppercase">{user.email}</span>
        </p>
      </div>
      )
}
      <div className="flex justify-evenly items-center mt-8 gap-4">
      {updateMode?<span className="hidden"></span>:<button
